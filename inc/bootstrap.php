 ...
 *   3. registering the generated autoloader, so the application's classes load
 *      on demand instead of being required up front;
 *   4. loading configuration, on top of safe defaults;
 *   5. putting the baseline security headers on the response;
 *   6. sending a plain-HTTP request on to the canonical HTTPS origin;
 *   7. starting the one and only session;
 *   8. handing out database connections, and deciding what a database failure
 *      does to the request.
 *
...
/*
 * The application's own classes, loaded on demand.
 *
 * inc/autoload/ is generated by Composer from the "autoload" section of
 * composer.json - tools/dump-autoload.sh is the exact command - and committed,
 * so the site needs nothing installed and no package of anybody else's to use
 * it. What it holds is a class map and the loader that reads it: Login,
 * Registration, PHPMailer, SMTP and phpmailerException, each pointing at the
 * file that declares it, and each file read only once the class in it is first
 * named. That is the whole of the gain: a request that never names PHPMailer
 * never reads the file that declares it.
 *
 * The fallback is what makes this additive. When the generated map is not
 * available - a tree that has this file but not that directory - the classes
 * are required up front exactly as inc/php-login.php used to require them, so
 * every page still works and the loading is no worse than it was before.
 * Nothing beyond loading is involved either way.
 */
$mcm_autoload_file = dirname(__FILE__) . '/autoload/autoload.php';

if (is_readable($mcm_autoload_file)) {
	require_once $mcm_autoload_file;
} else {
	require_once dirname(__FILE__) . '/libs/PHPMailer.php';
	require_once dirname(__FILE__) . '/classes/Login.php';
	require_once dirname(__FILE__) . '/classes/Registration.php';
}
unset($mcm_autoload_file);
