...
/*
 * ---------------------------------------------------------------------------
 * The proxy's policy half and its execution half
 * ---------------------------------------------------------------------------
 *
 * Two pages of this site reach the proxy without HTTP, and they need different
 * things from it. import_list.php reads a list on a visitor's behalf and wants
 * the whole policy asked again, refusal included: mcm_tmdb_resolve().
 * add_movie.php has already asked the method, the session, the token and the
 * list with its own guards before it needs a film's details, and has a page of
 * its own to answer, so it wants the execution half alone and a failure handed
 * back as a value: mcm_tmdb_execute().
 *
 * One thing separates them - what a refusal does - and everything below is
 * about keeping that one thing from becoming two policies. The behavioural half
 * drives both against the same absent session and the same operation, so the
 * difference is observed rather than described. The source half fixes which
 * calls each of them may contain, and which pages may call the execution half
 * at all, because "this page already has guards" must not be something a page
 * can assert about itself.
 *
 * That the execution half did not weaken add_movie.php is proven where it has
 * always been proven: 'add_movie resolves its own metadata over a real
 * database' refuses an anonymous add and a non-owner's add and shows the stub
 * received nothing.
 */
...
t_group('the proxy asks its policy on one half and not the other', array('server'), function () {
	...
	try {
		/* 1. The execution half, from a page with nobody behind it ---------- */

		$result = mcm_cli($fixture, 'tmdb_execute.php', array('MCM_TMDB_EXECUTE_MODE' => 'execute'));
		$report = mcm_report($result['stdout']);
		$at     = function ($key) use ($report) {
			return isset($report[$key]) ? $report[$key] : '(absent)';
		};

		t_same('no', $at('signed_in'), 'the page driving the execution half has nobody signed in behind it');

		// search's caller policy is "user". The execution half asks no policy
		// at all, so it answers anyway - which is the whole contract, and the
		// reason the written-down caller list below exists.
		t_same('yes', $at('search_ok'), 'the execution half serves an operation whose caller policy it never asks about');
		t_same('yes', $at('movie_ok'), 'and serves an ordinary operation');
		t_same('Fight Club', $at('movie_title'), 'answering the projection, not the upstream body');

		// Every way a plan can be refused comes back as a value.
		$refusals = array(
			'no_such_operation' => 'an operation that does not exist',
			'unknown_field'     => 'a field the operation does not accept',
			'bad_value'         => 'a value the operation will not take',
			'missing_value'     => 'a value the operation needs and did not get',
		);
		foreach ($refusals as $name => $description) {
			t_same('no', $at($name . '_ok'), $description . ' is refused');
			t_same('request', $at($name . '_category'), 'and comes back as the request category rather than ending the request');
			t_same('400', $at($name . '_status'), 'and carries the status a caller would answer with');
			t_same('present', $at($name . '_reason'), 'and a reason for the caller to log');
			t_same('', $at($name . '_title'), 'and no data at all');
		}

		// An upstream failure was already a value on both halves; it still is.
		t_same('no', $at('upstream_ok'), 'an upstream failure is refused too');
		t_same('upstream', $at('upstream_category'), 'and keeps its own category');

		// The assertion the whole mode exists for: nothing above ended the
		// request. A refusal that exited would take this line with it.
		t_same('yes', $at('reached_end'), 'not one of those refusals ended the request');

		/* 2. The policy half, from the same page and the same session ------- */

		$result = mcm_cli($fixture, 'tmdb_execute.php', array('MCM_TMDB_EXECUTE_MODE' => 'resolve'));
		$report = mcm_report($result['stdout']);

		t_same('no', isset($report['signed_in']) ? $report['signed_in'] : '(absent)', 'the page driving the policy half has nobody signed in behind it either');
		t_ok(!isset($report['reached_end']), 'the policy half ends the request instead of handing a refusal back');
		t_ok(!isset($report['resolve_search_ok']), 'so the caller never sees a result to report');
		t_contains(
			'{"error":"authentication_required","message":"You must be signed in to do that."}',
			$result['stdout'],
			'and the caller of the policy half gets the shared fixed body'
		);
		t_contains('no signed-in user', $result['log'], 'with the reason in the log');
	} catch (Throwable $error) {
		mcm_server_stop($stub);
		throw $error;
	}
...
t_group('neither half of the proxy can become the other', array('source'), function () {
	...
	// policy' proves about the order, for the one reason that matters here: if
	// these ever moved into the execution half, the two would stop being
	// different and the caller list below would stop meaning anything.
	$resolve = mcm_method_tokens($proxy, 'mcm_tmdb_resolve');
	t_ok(count($resolve) > 0, 'mcm_tmdb_resolve() is declared');
	t_same(1, mcm_count_calls_in($resolve, 'mcm_tmdb_require_session'), 'the policy half asks who is calling');
	t_same(1, mcm_count_calls_in($resolve, 'mcm_tmdb_require_owner'), 'and whose list it is');
	t_ok(mcm_count_calls_in($resolve, 'mcm_json_error') > 0, 'and refuses a request itself');

	// The execution half asks none of it. Read off its own tokens, so a call
	// elsewhere in the file cannot satisfy this.
	$execute = mcm_method_tokens($proxy, 'mcm_tmdb_execute');
	t_ok(count($execute) > 0, 'mcm_tmdb_execute() is declared');
	foreach (array('mcm_tmdb_require_session', 'mcm_tmdb_require_owner', 'mcm_require_login', 'mcm_require_list_owner') as $guard) {
		t_same(0, mcm_count_calls_in($execute, $guard), 'the execution half does not ask ' . $guard . '()');
	}
	// Nor does it end a request. This is the difference, stated as the absence
	// of the one call that would remove it.
	t_same(0, mcm_count_calls_in($execute, 'mcm_json_error'), 'the execution half refuses nothing itself');
	t_same(0, mcm_count_calls_in($execute, 'mcm_tmdb_respond'), 'and answers nothing itself');
	t_same(0, mcm_count_calls_in($execute, 'exit'), 'and ends nothing');

	// Both end at the same place, so there is still one path to TMDb.
	t_same(1, mcm_count_calls_in($execute, 'mcm_tmdb_run'), 'the execution half runs the operation through mcm_tmdb_run()');
	t_same(1, mcm_count_calls_in($resolve, 'mcm_tmdb_run'), 'and so does the policy half');
	t_same(0, mcm_count_calls_in($execute, 'mcm_tmdb_get'), 'neither reaches inc/tmdb.php on its own');
	t_same(0, mcm_count_calls_in($resolve, 'mcm_tmdb_get'), 'and neither does the other');

	// And who may call the execution half is a written-down list rather than
	// whatever the source happens to do, exactly as the guard adoption list is.
	$callers = array();
	foreach (mcm_php_sources(MCM_REPO_ROOT) as $file) {
		if ($file === $proxy) {
			continue;
		}
		if (mcm_count_calls($file, 'mcm_tmdb_execute') > 0) {
			$callers[] = substr($file, strlen(MCM_REPO_ROOT) + 1);
		}
	}
	sort($callers);
	t_same(mcm_tmdb_execute_callers(), $callers, 'the execution half is called by exactly the pages that have answered for it');

	// Each of those pages asks all four guards before it, and asks the policy
	// half nowhere - one page asking both would be asking the same questions
	// twice and answering them two different ways.
	foreach (mcm_tmdb_execute_callers() as $page) {
		$path   = MCM_REPO_ROOT . '/' . $page;
		$source = mcm_flat_source($path);

		t_ok(in_array($page, mcm_guarded_entry_points(), true), $page . ' is one of the endpoints that adopted the guards');
		t_same(0, mcm_count_calls($path, 'mcm_tmdb_resolve'), $page . ' does not also ask the policy half');
		foreach (mcm_guard_functions() as $guard) {
			t_ok(mcm_count_calls($path, $guard) > 0, $page . ' asks ' . $guard . '() before it may use the execution half');
			t_ok(
				strpos($source, $guard) < strpos($source, 'mcm_tmdb_execute'),
				$page . ' asks ' . $guard . '() before it calls the execution half'
			);
		}
	}

	// The page that has not answered for it still uses the policy half, and the
	// entry point still holds no policy of its own. Both are issue #38's, and
	// both have to survive this.
	$import = MCM_REPO_ROOT . '/import_list.php';
	t_same(1, mcm_count_calls($import, 'mcm_tmdb_resolve'), 'import_list.php still reads its list through the policy half');
	t_same(0, mcm_count_calls($import, 'mcm_tmdb_execute'), 'and does not reach past it');
	$serve = mcm_method_tokens($proxy, 'mcm_tmdb_serve');
	t_same(1, mcm_count_calls_in($serve, 'mcm_tmdb_resolve'), 'the entry point still settles a request through the policy half');
...
function mcm_mail_cases(array $runtime)
{
	...
	try {
		...
		// The other half of the loading claim the "generated class autoloader"
		// group makes without a send: the mail library is not read by loading
		// the bootstrap and php-login, and it is read by the time a send has
		// been attempted - whichever way that attempt ended on this runtime.
		t_same('no', mcm_at($report, 'phpmailer_before', ''), 'the page had not read the mail library before it sent anything' . $where);
		t_same('no', mcm_at($report, 'smtp_before', ''), 'nor the SMTP client beside it' . $where);
		t_same('yes', mcm_at($report, 'phpmailer_after', ''), 'and the send is what read it' . $where);
		t_same('yes', mcm_at($report, 'smtp_after', ''), 'and the SMTP transport is what read the client' . $where);
...
/*
 * ---------------------------------------------------------------------------
 * 27. What a request loads, and the generated map that decides it
 * ---------------------------------------------------------------------------
 *
 * Sixteen entry points used to open by requiring inc/php-login.php, and that
 * one file required inc/libs/PHPMailer.php - 2,949 lines - on every request
 * that reached any of them: an anonymous view of a public sharing link, every
 * list action, every movie action. None of those requests sends mail. Only
 * registration and password reset do.
 *
 * They are loaded from a class map now, generated by Composer from the files
 * this project already had and committed so that a checkout with nothing
 * installed still works. The two groups below are the two halves of that
 * claim. The first reads the committed map and the source around it: that it
 * covers every class those directories declare, that nothing requires a class
 * file by hand any more, and that every include names its target relative to
 * the file that names it rather than to whatever directory the process happens
 * to be in. The second drives the real pages and reads what each request
 * actually left declared - including the pages that refuse and exit, which is
 * why the reporter is a shutdown function rather than an appended file.
 *
 * The second group also drives every one of those pages against a fixture with
 * the generated map removed, because "additive" has to mean something that can
 * be checked: a tree that has inc/bootstrap.php but not inc/autoload/ falls
 * back to requiring the same three files up front, which is exactly the loading
 * this change replaced, and every page keeps working.
 */

t_group('generated class autoloader', array('source'), function () {
	$map = mcm_autoload_classmap(MCM_REPO_ROOT);

	t_ok(count($map) > 0, 'the committed class map was found and returns an array', count($map) . ' entries');
	t_ok(is_readable(MCM_REPO_ROOT . '/inc/autoload/autoload.php'), 'and the loader that reads it is committed beside it');

	// Composer writes its own metadata class into every map it generates. No
	// package is installed here, so no file for it is written - and nothing in
	// this project names it, which is what makes the entry unreachable rather
	// than merely unused. Checked, not assumed.
	$composer_own = 'Composer\\InstalledVersions';
	foreach (mcm_php_sources(MCM_REPO_ROOT) as $file) {
		t_lacks('InstalledVersions', mcm_flat_source($file), substr($file, strlen(MCM_REPO_ROOT) + 1) . ' names none of Composer\'s own classes');
	}

	// Every class the site declares in the directories the map covers is in it,
	// pointing at the file that declares it. This is what fails when a class is
	// added, renamed or moved and tools/dump-autoload.sh was not re-run.
	$declared = array();
	foreach (mcm_autoloaded_directories() as $directory) {
		foreach (glob(MCM_REPO_ROOT . '/' . $directory . '/*.php') as $file) {
			foreach (mcm_declared_classes($file) as $class) {
				$declared[$class] = $file;
			}
		}
	}

	t_ok(count($declared) >= 5, 'the covered directories declare the classes this site has', implode(', ', array_keys($declared)));
	foreach ($declared as $class => $file) {
		$mapped = isset($map[$class]) ? realpath($map[$class]) : '';
		t_same(realpath($file), $mapped, $class . ' is mapped to the file that declares it');
	}

	// And nothing is in the map that is not on disk, so no class can be named
	// and then fail to load. Composer's own entry is the one exception, and it
	// is named here rather than skipped silently.
	foreach ($map as $class => $file) {
		if ($class === $composer_own) {
			t_ok(!file_exists($file), 'Composer\'s own metadata class is mapped to a file no package installed, and nothing names it', $file);
			continue;
		}
		t_ok(is_readable($file), $class . ' is mapped to a file this checkout carries', $file);
		t_ok(in_array($class, mcm_declared_classes($file), true), 'and that file is where ' . $class . ' is declared');
	}

	// The map is the whole answer: a class that is not in it is never hunted
	// for on disk. Read off the generated loader rather than assumed, because
	// which flags generated it is the thing that decides this.
	t_contains(
		'setClassMapAuthoritative ( true )',
		mcm_flat_source(MCM_REPO_ROOT . '/inc/autoload/composer/autoload_real.php'),
		'the generated loader answers from the map alone'
	);

	/* The bootstrap is where it is registered, and how it degrades ---------- */

	$bootstrap = MCM_REPO_ROOT . '/inc/bootstrap.php';
	$flat_boot = mcm_flat_source($bootstrap);
	$requires  = array();
	foreach (mcm_include_statements($bootstrap) as $statement) {
		foreach ($statement['literals'] as $literal) {
			$requires[] = $literal;
		}
	}

	// The path is built one line above the require, the way the configuration
	// file's already is, so the statement itself carries a variable and the
	// anchoring is in the assignment. Both halves are checked.
	t_same(
		1,
		substr_count($flat_boot, "\$mcm_autoload_file = dirname ( __FILE__ ) . '/autoload/autoload.php'"),
		'the bootstrap names the generated autoloader once, anchored to its own file'
	);
	t_same(1, substr_count($flat_boot, 'require_once $mcm_autoload_file'), 'and requires it once');
	t_contains('is_readable ( $mcm_autoload_file )', $flat_boot, 'only after asking whether it is there');

	// The fallback is the additive half, and it has to name the same three
	// files the entry points used to get from inc/php-login.php.
	foreach (array('/libs/PHPMailer.php', '/classes/Login.php', '/classes/Registration.php') as $fallback) {
		t_ok(in_array($fallback, $requires, true), 'the bootstrap can still require ' . $fallback . ' if the generated map is unavailable');
	}

	/* php-login.php requires nothing but the bootstrap any more ------------- */

	$php_login = MCM_REPO_ROOT . '/inc/php-login.php';
	foreach (mcm_include_statements($php_login) as $statement) {
		t_ok($statement['anchored'], 'every include in inc/php-login.php is anchored to that file: ' . $statement['text']);
	}

	$flat = mcm_flat_source($php_login);
	t_lacks('libs/PHPMailer.php', $flat, 'inc/php-login.php no longer requires the mail library');
	t_lacks('classes/Login.php', $flat, 'nor the Login class');
	t_lacks('classes/Registration.php', $flat, 'nor the Registration class');
	t_lacks('password_compatibility_library', $flat, 'nor the password compatibility library');
	t_lacks('5.3.7', $flat, 'and the minimum version it declares is no longer the one it never needed');
	t_contains("'7.0.0'", $flat, 'the declared minimum is 7.0.0');

	// 7.0.0 is not a guess: it is what the code's own syntax and functions
	// require. Both markers are checked, so a floor raised or lowered without
	// the code moving with it fails here.
	t_contains('<=>', file_get_contents(MCM_REPO_ROOT . '/inc/dialog_trailers.php'), 'the spaceship operator, PHP 7.0, is in the code the floor has to admit');
	t_ok(mcm_count_calls(MCM_REPO_ROOT . '/inc/security.php', 'random_bytes') > 0, 'and random_bytes(), PHP 7.0, is called by the security primitives');

	/* The compatibility library is gone, and nothing looks for it ----------- */

	t_ok(!file_exists(MCM_REPO_ROOT . '/inc/libs/password_compatibility_library.php'), 'inc/libs/password_compatibility_library.php is gone');
	foreach (mcm_php_sources(MCM_REPO_ROOT) as $file) {
		t_lacks('password_compatibility', mcm_flat_source($file), substr($file, strlen(MCM_REPO_ROOT) + 1) . ' does not look for the removed compatibility library');
	}

	/* Nobody requires a class file by hand, and nobody uses the include path - */

	$vendored = array('inc/libs/PHPMailer.php', 'inc/libs/class.smtp.php');
	foreach (mcm_php_sources(MCM_REPO_ROOT) as $file) {
		$name = substr($file, strlen(MCM_REPO_ROOT) + 1);
		if (in_array($name, $vendored, true)) {
			// The vendored library's own includes are its own business: this
			// change does not touch what it does, only when it is read.
			continue;
		}

		foreach (mcm_include_statements($file) as $statement) {
			// The bootstrap builds two of its paths a line above the require -
			// the configuration file's and the generated autoloader's - so the
			// statement itself carries a variable. Both assignments are
			// anchored, and both are checked above.
			$by_variable = ($name === 'inc/bootstrap.php' && count($statement['literals']) === 0);
			t_ok($statement['anchored'] || $by_variable, $name . ' anchors its include to its own file: ' . $statement['text']);
		}

		if ($name === 'inc/bootstrap.php') {
			continue;
		}
		foreach (mcm_include_statements($file) as $statement) {
			foreach ($statement['literals'] as $literal) {
				$class_file = (strpos($literal, 'classes/') !== false || strpos($literal, 'libs/') !== false);
				t_ok(!$class_file, $name . ' does not require a class file by hand: ' . $statement['text']);
			}
		}
	}

	/* The sixteen pages that load php-login, written down ------------------- */

	foreach (mcm_php_login_users() as $name) {
		$file = MCM_REPO_ROOT . '/' . $name;
		$found = false;
		foreach (mcm_include_statements($file) as $statement) {
			foreach ($statement['literals'] as $literal) {
				if (substr($literal, -13) === 'php-login.php') {
					$found = true;
					t_ok($statement['anchored'], $name . ' loads php-login anchored to its own directory');
				}
			}
		}
		t_ok($found, $name . ' still loads inc/php-login.php');
	}
});

t_group('what a request loads', array('fixture'), function () {
	// Every page here is driven exactly as it is committed. The reporter is
	// prepended with an ini setting, so nothing in the fixture's copy of the
	// site differs from the checkout's.
	$pages = array(
		// Pages that name a class, and get it from the map.
		'index.php'          => array('Login'),
		'about.php'          => array('Login'),
		'login.php'          => array('Login'),
		'edit.php'           => array('Login'),
		'password_reset.php' => array('Login'),
		'register.php'       => array('Registration'),
		// Pages that name none, and now read none. Each of these is a request
		// that used to read the mail library and had no use for it.
		'create_list.php'    => array(),
		'rename_list.php'    => array(),
		'delete_list.php'    => array(),
		'adjust_lists.php'   => array(),
		'share_lists.php'    => array(),
		'delete_movie.php'   => array(),
		'move.php'           => array(),
		'add_movie.php'      => array(),
		'logout.php'         => array(),
	);

	$fixture  = mcm_fixture('loading');
	$prepend  = array('auto_prepend_file' => $fixture['public'] . '/declared_classes.php');
	$loaded   = array();

	foreach ($pages as $page => $classes) {
		$result = mcm_cli($fixture, $page, array(), array('ini' => $prepend));
		$report = mcm_report($result['stdout']);
		$loaded[$page] = $result;

		t_ok(count($report) > 0, $page . ' reported what it had loaded', substr($result['stdout'], -200));
		t_same('yes', mcm_loading_value($report, 'read_inc/autoload/autoload.php'), $page . ' registered the generated autoloader');

		// The finding this whole change is about.
		t_same('no', mcm_loading_value($report, 'class_PHPMailer'), $page . ' left the mail library undeclared');
		t_same('no', mcm_loading_value($report, 'class_SMTP'), $page . ' left the SMTP client undeclared');
		t_same('no', mcm_loading_value($report, 'read_inc/libs/PHPMailer.php'), $page . ' never read inc/libs/PHPMailer.php');
		t_same('no', mcm_loading_value($report, 'read_inc/libs/class.smtp.php'), $page . ' never read inc/libs/class.smtp.php');

		foreach (array('Login', 'Registration') as $class) {
			$expected = in_array($class, $classes, true) ? 'yes' : 'no';
			t_same($expected, mcm_loading_value($report, 'class_' . $class), $page . ' ' . ($expected === 'yes' ? 'got ' : 'never read ') . $class);
		}
	}

	/* The same pages, with the generated directory taken away -------------- */

	// The same site with the generated map unavailable. The bootstrap's
	// fallback requires the three files php-login.php used to require, which is
	// the loading this change replaced: every page keeps working, and the mail
	// library is read again exactly as it always was.
	$partial = mcm_fixture('loading-partial');
	mcm_rmtree($partial['public'] . '/inc/autoload');
	t_ok(!file_exists($partial['public'] . '/inc/autoload/autoload.php'), 'the fixture with the change only partly in place has no generated autoloader');
	$partial_prepend = array('auto_prepend_file' => $partial['public'] . '/declared_classes.php');

	foreach ($pages as $page => $classes) {
		$result = mcm_cli($partial, $page, array(), array('ini' => $partial_prepend));
		$report = mcm_report($result['stdout']);

		t_same('no', mcm_loading_value($report, 'read_inc/autoload/autoload.php'), $page . ' finds no generated autoloader to register');
		t_same('yes', mcm_loading_value($report, 'class_Login'), $page . ' still has Login, required up front as before');
		t_same('yes', mcm_loading_value($report, 'class_Registration'), $page . ' still has Registration');
		t_same('yes', mcm_loading_value($report, 'class_PHPMailer'), $page . ' reads the mail library again, which is the behaviour this replaced');

		// Nothing is broken by the missing directory: the page ends the way it
		// ends with it, and says the same thing to whoever asked.
		t_same($loaded[$page]['status'], $result['status'], $page . ' ends the same way with the generated map unavailable');
		t_same(
			mcm_loading_body($loaded[$page]['stdout']),
			mcm_loading_body($result['stdout']),
			$page . ' answers with exactly the same page either way'
		);
		// The refusal endpoints log their refusal either way, so the claim is
		// not "logs nothing" but "logs nothing more, and nothing about
		// loading": a fallback that half worked would say so here.
		t_lacks('autoload', $result['log'], $page . ' logs nothing about the unavailable generated map');
		t_lacks('Failed opening', $result['log'], $page . ' opened every file it asked for');
		t_lacks('not found', $result['log'], $page . ' found every class it named');
		t_same(
			mcm_loading_log_lines($loaded[$page]['log']),
			mcm_loading_log_lines($result['log']),
			$page . ' logs the same lines of its own as it does with the generated map in place'
		);
	}
});

/**
 * How many lines of a page's own the log holds.
 *
 * Diagnostics raised inside the vendored mail library are left out, and the
 * reason is the point of the comparison they are left out of: reading that
 * library again is exactly what the fallback does, so on a runtime that
 * deprecates something inside it - PHP 8.5 and the (boolean) casts at
 * inc/libs/PHPMailer.php:776 and :779 - the extra lines are that library's,
 * not the page's. Counting them would turn "the fallback changes nothing this
 * page does" into a statement about which runtime ran it.
 */
function mcm_loading_log_lines($log)
{
	$lines = 0;
	foreach (explode("\n", $log) as $line) {
		if (strpos($line, '[mcm]') !== false && strpos($line, '/inc/libs/') === false) {
			$lines++;
		}
	}
	return $lines;
...
/**
 * One reported value, by its exact key.
 *
 * mcm_at() reads a dotted path into a decoded body, and half the keys here are
 * file names with a dot in them; this is the flat lookup those want.
 */
function mcm_loading_value(array $report, $key)
{
	return isset($report[$key]) ? $report[$key] : '';
}

/**
 * A page's own output, with the loading report cut off the end.
 *
 * The reporter prints from a shutdown function, so its lines are always the
 * tail. Cutting them off is what lets two runs be compared as pages rather
 * than as reports.
 */
function mcm_loading_body($stdout)
{
	$at = strpos($stdout, "\nclass_Login=");

	return ($at === false) ? $stdout : substr($stdout, 0, $at);
}
